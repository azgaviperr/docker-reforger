...
def random_passphrase():
    ...
    words = []
    with open("/usr/share/dict/american-english", encoding="utf-8") as word_file:
        words = [word.strip() for word in word_file if len(word.strip()) >= 4]

    # Generate a passphrase with two random words and a random separator
    ...
    return f"{word1}{separator}{word2}{random_number}{special_char}"
...
if os.environ["ARMA_CONFIG"] != "docker_generated":
    ...
    if env_defined("RCON_PASSWORD"):
        ...
        rconPassword = random_passphrase()
        print(f"\033[92m[INFO]\033[0m Autogenerated RCON password: \033[93m{rconPassword}\033[0m")
    ...
    if env_defined("GAME_PASSWORD_ADMIN"):
        ...
        print(f"\033[92m[INFO]\033[0m Autogenerated Admin password: \033[93m{adminPassword}\033[0m")
